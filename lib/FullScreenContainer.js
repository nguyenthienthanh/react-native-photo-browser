...
export default class FullScreenContainer extends React.Component {
  ...
  static propTypes = {
    ...
    /*
     * refresh the list to apply image change
     */
  ...
  _updatePageIndex(index) {
    ...
    this.setState({
      ...
      const totalCount = this.props.dataSource.getRowCount()
      const newTitle = customTitle ? customTitle(nextIndex, totalCount) : `${nextIndex} of ${totalCount}`;

      this.props.updateTitle(newTitle, index);
      this.props.onChangeImage(index, this.state.currentMedia);
    });
  }
  ...
  _renderScrollableContent() {
    const { dataSource, mediaList, listViewProps={} } = this.props;
